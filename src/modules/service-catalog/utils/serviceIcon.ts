 ...
 * can *reference* an icon. A reference is either:
 *
 ...
 *     `arpa-h-logomark.svg`. Use this for our own marks, or when a vendor does
 *     not publish an Iconify logo.
 *
 ...
 *   3. the uploaded thumbnail image (thumbnail_url)  (legacy fallback)
 *   4. a generic default icon
 */

import { SERVICE_ICON_MAP } from "../constants/serviceIconMap";
...
/**
 * Classify a raw reference string into a renderable icon, or `null` if it is
 * neither a recognized Iconify name nor an asset filename. Shared by both the
 * marker and the curated map so they behave identically.
 */
...
export function resolveItemIcon(input: {
  ...
  // 3. Legacy uploaded thumbnail image.
